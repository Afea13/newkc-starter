...
const mapSizeToMuiSize = (size: `${number}` | number | undefined): "sm" | "md" | "lg" | undefined => {
    if (size === undefined) return undefined;

    const numericSize = typeof size === "string" ? parseInt(size, 10) : size;

    if (numericSize <= 20) return "sm"; // Example mapping: small sizes map to "sm"
    if (numericSize <= 40) return "md"; // Medium sizes map to "md"
    return "lg"; // Larger sizes map to "lg"
};
    ...
    return (
        <>
            <Input
                ...
                size={mapSizeToMuiSize(attribute.annotations.inputTypeSize)}
                maxLength={
