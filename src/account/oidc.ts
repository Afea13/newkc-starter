...
let realm: string = "myrealm"; // Default value for realm

export const { OidcProvider, useOidc, getOidc } = (() => {
  const isAuthGloballyRequired = true;

  const publicUrl = undefined;//import.meta.env.BASE_URL;
  ...
  const getParam = (params: { name: string; defaultValue: string }) => {
    ...
    realm = getParam({ name: "realm", defaultValue: "myrealm" });
  ...
  const clientId = getParam({ name: "client", defaultValue: "account-console" });
